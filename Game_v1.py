...
while game:
    ...
    if keys[K_r] and finish == True:
        finish = False
        PlasmaBall.rect.x = 310 
        PlasmaBall.rect.y = 220
        mixer.music.play()
    ...
    if finish != True:
        ...
        if PlasmaBall.rect.x < RPlat.rect.x - 52:
            ...
            window.blit(text, (62, 150))
            text2 = font1.render('"R" to replay.', True, (255, 0, 0))
            window.blit(text2, (205, 250))
            mixer.music.stop()
            end.play()
            finish = True
            #game = False
        if PlasmaBall.rect.x > LPlat.rect.x + 70:
            font1 = font.SysFont('verdana', 40)
            text = font1.render('Game Over\nRight player lose.', True, (0, 0, 255))
            window.blit(text, (38, 200))
            text2 = font1.render('"R" to replay.', True, (0, 0, 255))
            window.blit(text2, (205, 250))
            mixer.music.stop()
            end.play()
            finish = True
